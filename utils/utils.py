...
def load_dataset(path, sampling_rate, release=False):
    # Verwenden Sie nur PTBXL-Datensatz
    if path.split('/')[-2] == 'ptbxl':
        ...
        # Andernfalls können Sie X und Y mit leeren Werten initialisieren oder eine benutzerdefinierte Logik hinzufügen
        X, Y = None, None
...
def load_raw_data_icbeb(df, sampling_rate, path):
    # Statt den ICBEB-Datensatz zu laden, können Sie hier eine benutzerdefinierte Logik für den PTBXL-Datensatz hinzufügen
    # Beachten Sie, dass Sie hier möglicherweise zusätzliche Logik benötigen, um den PTBXL-Datensatz richtig zu laden.
    X = load_raw_data_ptbxl(df, sampling_rate, path)
    return X
